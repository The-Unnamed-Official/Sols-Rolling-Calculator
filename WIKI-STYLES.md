...
- Wiki artwork applies only to aura names. Rarity numbers, counts, True Chance, subtitles, and native-roll labels use the aura's tier colors. Accessible names are separate from decorative duplicate layers; searching and copying use the canonical names.
- The Full Aura Style Rework switch at the bottom of Preferences is on by default. Turning it off immediately restores tier-colored aura names across existing and future titles, including changelog entries and image exports. The choice persists across reloads; item styling is independent.
- Tier headings mark visible tier boundaries in aura filters, rarity-sorted results, and shared images. They follow searching and paging, remain outside the roll history, and disappear when the rework is switched off. Chronological and alphabetical feeds retain their existing order without tier headings.
- The in-app Credits Directory includes a searchable Artwork entry for Sol's RNG Wiki contributors and links to the Wiki and its licensing terms.
